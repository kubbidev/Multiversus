...
public abstract class AbstractJavaScheduler implements SchedulerAdapter {
    ...
    private final MultiBootstrap bootstrap;

    private final ScheduledThreadPoolExecutor scheduler;
    private final ForkJoinPool worker;

    public AbstractJavaScheduler(MultiBootstrap bootstrap) {
        this.bootstrap = bootstrap;

        this.scheduler = new ScheduledThreadPoolExecutor(1, r -> {
            Thread thread = Executors.defaultThreadFactory().newThread(r);
            thread.setName("multiversus-scheduler");
            return thread;
        });
    ...
    public void shutdownScheduler() {
        ...
        try {
            if (!this.scheduler.awaitTermination(1, TimeUnit.MINUTES)) {
                this.bootstrap.getPluginLogger().severe("Timed out waiting for the Multiversus scheduler to terminate");
                reportRunningTasks(thread -> thread.getName().equals("multiversus-scheduler"));
            }
    ...
    public void shutdownExecutor() {
        ...
        try {
            if (!this.worker.awaitTermination(1, TimeUnit.MINUTES)) {
                this.bootstrap.getPluginLogger().severe("Timed out waiting for the Multiversus worker thread pool to terminate");
                reportRunningTasks(thread -> thread.getName().startsWith("multiversus-worker-"));
            }
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    private static final class WorkerThreadFactory implements ForkJoinPool.ForkJoinWorkerThreadFactory {
        ...
        public ForkJoinWorkerThread newThread(ForkJoinPool pool) {
            ...
            thread.setName("multiversus-worker-" + COUNT.getAndIncrement());
            return thread;
        }
    }
