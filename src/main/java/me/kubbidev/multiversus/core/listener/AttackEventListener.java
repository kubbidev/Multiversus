package me.kubbidev.multiversus.core.listener;

import me.kubbidev.multiversus.FBukkitPlugin;
import me.kubbidev.multiversus.core.damage.AttackMetadata;
import me.kubbidev.multiversus.core.damage.DamageType;
import me.kubbidev.multiversus.core.event.attack.AttackEvent;
import me.kubbidev.multiversus.core.event.attack.EntityAttackEvent;
import me.kubbidev.multiversus.core.event.attack.EntityKillEntityEvent;
 ...
 * <p>
 * Basically Multiversus is monitoring every single attack from every single entity to keep track:
...
public class AttackEventListener implements Listener {
    ...
    public AttackEventListener(FBukkitPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void registerAttacks(EntityDamageEvent e) {
        // ignore fake events...
        if (!(e.getEntity() instanceof LivingEntity) || this.plugin.getFakeEventManager().isFake(e)) {
            return;
        }
        ...
        if (attack.hasAttacker()) {
            ...
            LivingEntity entity = attack.getAttacker().getEntity();
        ...
        if (attack.hasAttacker() && e.getFinalDamage() >= living.getHealth()) {
            EntityKillEntityEvent event = new EntityKillEntityEvent(attack, living);
            event.callEvent();
        }
    }
}